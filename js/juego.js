...
var instrucciones = [
  "Reubicar las piezas desordenadas para formar la imagen del objetivo"
];
...
function chequearSiGano() {
  var posicionCorrecta = [[1, 2, 3], [4, 5, 6], [7, 8, 9]];
  var iguales = false;
  if (JSON.stringify(grilla) == JSON.stringify(posicionCorrecta)) {
    iguales = true;
  }
  return iguales;
}
...
function mostrarCartelGanador() {
  ...
  Swal.fire({
    ...
    width: 500,
    padding: '3em',
    background: `#fff url(./images/minions-2.gif) no-repeat`,
    backdrop: `
    rgba(0,0,123,0.4)
    url("./images/gameboy.gif")
    center left
    no-repeat
  `
  });
...
function posicionValida(fila, columna) {
  ...
  var valido = true;
  var indiceFila = fila;
  var indiceColumna = columna;


  if (indiceFila > 2 || indiceColumna > 2) {
    valido = false
  } else if (indiceFila < 0 || indiceColumna < 0) {
    valido = false;
  }
...
function moverEnDireccion(direccion) {
  ...
  // Mueve la pieza hacia la de abajo, reemplazandola con la blanca
